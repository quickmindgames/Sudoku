...
import com.gamestudio.sudo.domain.model.DifficultyStatistics
import com.gamestudio.sudo.domain.model.GameStatistics
import com.gamestudio.sudo.domain.model.StreakStatistics
...
fun OverallStatsCard(stats: GameStatistics) {
    Card(
        ...
        Column(modifier = Modifier.padding(20.dp)) {
            ...
            StatisticRow("Flaw-Free Wins", stats.flawFreeWins.toString(), MaterialTheme.colorScheme.tertiary)
        }
    }
}
...
fun DifficultyStatsTab(difficulty: DifficultyStatistics) {
    Column {
        ...
        val difficultyEmoji = when (difficulty.difficulty) {
            "Breeze" -> "🌬️"
            "Pulse" -> "💓"
            "Rage" -> "🔥"
            "Elite" -> "⭐"
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(
                    text = "${difficulty.difficulty} – ${feedback.title}",
                    fontSize = 18.sp,
                    fontWeight = FontWeight.Bold,
                    color = MaterialTheme.colorScheme.primary
                )
...
fun StreakStatsTab(stats: StreakStatistics) {
    Column {
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(
                    text = "Streak Challenge – ${feedback.title}",
                    fontSize = 18.sp,
                    fontWeight = FontWeight.Bold,
                    color = MaterialTheme.colorScheme.primary
                )
...
fun StreakStatsCard(stats: StreakStatistics) {
    Card(
        ...
        Column(modifier = Modifier.padding(20.dp)) {
            ...
            StatisticRow("Best Time", stats.bestTime?.let { formatTime(it) } ?: "—")
            StatisticRow("Best Score", stats.bestScore?.toString() ?: "—")
            StatisticRow("Highest Streak Day", stats.highestStreakDay.toString(), MaterialTheme.colorScheme.tertiary)
            StatisticRow("Current Consecutive Wins", stats.currentConsecutiveWins.toString(), MaterialTheme.colorScheme.primary)
        }
    }
}
